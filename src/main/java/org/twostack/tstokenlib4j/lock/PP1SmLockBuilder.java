 ...
 *
 * <p>A state-machine token models a multi-step workflow between an operator and a
 * counterparty. The script encodes the current state, the allowed state transitions
 * (as a bitmask), a checkpoint count, a commitment hash, and a timeout delta,
 * alongside the public-key hashes that identify the owner, operator, and
 * counterparty.</p>
 *
 ...
 * ({@code currentState}, {@code checkpointCount}, {@code transitionBitmask}) or as 4-byte
...
public class PP1SmLockBuilder extends LockingScriptBuilder {
     ...
     * @param operatorPKH       the HASH160 of the operator's public key (must be exactly 20 bytes)
     * @param counterpartyPKH   the HASH160 of the counterparty's public key (must be exactly 20 bytes)
     * @param currentState      the current state index, encoded as a single byte
     * @param checkpointCount    the total number of checkpoints, encoded as a single byte
    ...
    public PP1SmLockBuilder(byte[] ownerPKH, byte[] tokenId,
        ...
        if (operatorPKH == null || operatorPKH.length != 20) {
            throw new IllegalArgumentException("operatorPKH must be 20 bytes");
        }
        if (counterpartyPKH == null || counterpartyPKH.length != 20) {
            throw new IllegalArgumentException("counterpartyPKH must be 20 bytes");
        }
        ...
        this.operatorPKH = operatorPKH.clone();
        this.counterpartyPKH = counterpartyPKH.clone();
     ...
     * {@code {{ownerPKH}}}, {@code {{tokenId}}}, {@code {{operatorPKH}}},
     * {@code {{counterpartyPKH}}}, {@code {{rabinPubKeyHash}}}, {@code {{currentState}}}, {@code {{checkpointCount}}},
    ...
    public Script getLockingScript() {
        ...
        hex = hex.replace("{{operatorPKH}}", Utils.HEX.encode(operatorPKH));
        hex = hex.replace("{{counterpartyPKH}}", Utils.HEX.encode(counterpartyPKH));
        hex = hex.replace("{{rabinPubKeyHash}}", Utils.HEX.encode(rabinPubKeyHash));
        hex = hex.replace("{{currentState}}", encodeHexByte(currentState));
        hex = hex.replace("{{checkpointCount}}", encodeHexByte(checkpointCount));
    ...
    /** @return a defensive copy of the operator's public-key hash (20 bytes) */
    public byte[] getOperatorPKH() { return operatorPKH.clone(); }
    /** @return a defensive copy of the counterparty's public-key hash (20 bytes) */
    public byte[] getCounterpartyPKH() { return counterpartyPKH.clone(); }
